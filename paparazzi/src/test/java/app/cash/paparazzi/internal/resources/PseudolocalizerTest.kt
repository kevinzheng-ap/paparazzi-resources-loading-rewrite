...
class PseudolocalizerTest() {
  ...
  fun testPlainTextAccent() {
    ...
    simpleTest("Try searching for another name or <xliff:g id=\"cashtag_symbol\" example=\"\$j\">%2\$d</xliff:g>Cashtag", "[Ţŕý šéåŕçĥîñĝ ƒöŕ åñöţĥéŕ ñåḿé öŕ <xliff:g id=\"cashtag_symbol\" example=\"\$j\">»%2\$d«</xliff:g>Çåšĥţåĝ one two three four five]", Pseudolocalizer.Method.ACCENT)
  ...
  fun testPlainTextBidi() {
    ...
    simpleTest("Try searching for another name or <xliff:g id=\"cashtag_symbol\" example=\"\$j\">%2\$d</xliff:g>Cashtag", "${bidiWordStart}Try$bidiWordEnd ${bidiWordStart}searching$bidiWordEnd ${bidiWordStart}for$bidiWordEnd ${bidiWordStart}another$bidiWordEnd ${bidiWordStart}name$bidiWordEnd ${bidiWordStart}or$bidiWordEnd $bidiWordStart<xliff:g$bidiWordEnd ${bidiWordStart}id=\"cashtag_symbol\"$bidiWordEnd ${bidiWordStart}example=\"\$j\">%2\$d</xliff:g>Cashtag$bidiWordEnd", Pseudolocalizer.Method.BIDI)
    compoundTest(
      "${bidiWordStart}hello$bidiWordEnd\n ${bidiWordStart}world$bidiWordEnd\n",
      Pseudolocalizer.Method.BIDI,
      "hello",
  ...
  fun testICUBidi() {
    ...
    simpleTest(
      "{placeholder}",
      "$bidiWordStart{placeholder}$bidiWordEnd",
      Pseudolocalizer.Method.BIDI
    )
    simpleTest(
  ...
  fun testHtmlTags() {
    ...
    // CDATA tags should not be translated, but the inner content (i.e."world")
    // should be.
    simpleTest(
      "<![CDATA[<html>world</html>]]",
      "[<![CDATA[<html>ŵöŕļð</html>]] one two]",
      Pseudolocalizer.Method.ACCENT
    )
  }
}
